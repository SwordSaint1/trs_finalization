...
if ($method == 'fetch_sched') {
	...
	$shift = $_POST['shift'];
	$c = 0;
	$query = "SELECT *,TIME_FORMAT(start_time, '%H:%i:%s') as start_time, TIME_FORMAT(end_time, '%H:%i:%s') as end_time FROM trs_training_sched WHERE start_date LIKE '$start%' AND shift LIKE '$shift%' AND slot >=1 AND sched_stat = 2 AND start_date >= '$server_date_only' AND training_type = 'SB-Special Condition'";
