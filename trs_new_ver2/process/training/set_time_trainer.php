...
if($method == 'set'){
        ...
        if($stmt->rowCount() > 0){
            foreach($stmt->fetchALL() as $x){
                ...
                echo '<tr style="cursor:pointer;" class="modal-trigger" data-toggle="modal" data-target="#set" onclick="get_set(&quot;'.$x['id'].'~!~'.$x['shift'].'~!~'.$x['training_type'].'~!~'.$x['slot'].'~!~'.$x['training_code'].'~!~'.$x['sched_stat'].'&quot;)">'; 
                echo '<td>'.$c.'</td>';
...
if($method == 'prevbatchApp_trainingedit'){
        ...
        if($stmt->rowCount() > 0){
            foreach($stmt->fetchALL() as $x){
               echo $x['id'].'~!~'.$x['training_code'].'~!~'.$x['sched_stat'].'~!~'.$x['training_type'].'~!~'.$x['process'].'~!~'.$x['start_date'].'~!~'.$x['start_time'].'~!~'.$x['end_date'].'~!~'.$x['end_time'].'~!~'.$x['slot'].'~!~'.$x['shift'].'~!~'.$x['location'];
            }
        }
}
